...
class ChatBotApp:
    ...
    def create_widgets(self):
        ...
        self.chat_history = scrolledtext.ScrolledText(self.root, width=50, height=20, state='disabled',font=tfont, bg="lightgreen")
        ...
        # Video widget
        video_path = './templates/test..mp4'  # Replace with the actual path to your video
        self.add_video_widget(video_path)

    def add_video_widget(self, video_path):
        video_frame = ttk.Frame(self.root)
        video_frame.grid(row=3, column=0, columnspan=3, padx=10, pady=10)

        # Create a Canvas widget to embed the videoS
        video_canvas = tk.Canvas(video_frame, width=600, height=400)
        video_canvas.grid(row=0, column=0)
        ...
        # Embed the pygame display into the Canvas
        video_id = video_canvas.create_window(
            0, 0, anchor=tk.NW, window=pygame.display.get_wm_info()['window'])
    ...
    def send_message(self):
        ...
        if self.count==True:
            pygame.mixer.init()
            print("playing fore")
            self.play_sound('./static/fore.mp3')
            time.sleep(5)
            self.count=False
